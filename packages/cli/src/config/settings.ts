...
export function getSystemSettingsPath(): string {
  if (process.env.UEVO_CLI_SYSTEM_SETTINGS_PATH) {
    return process.env.UEVO_CLI_SYSTEM_SETTINGS_PATH;
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/GeminiCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\uevo-cli\\settings.json';
  } else {
    return '/etc/uevo-cli/settings.json';
  }
}
