...
import * as ServerConfig from '@uevo/uevo-cli-core';
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/UEVO.md'],
      },
      {
        config: {
          name: 'ext2',
  ...
  it('should correctly use mocked homedir for global path', async () => {
    ...
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_GEMINI_DIR_LOCAL, 'UEVO.md');
    mockFs({
...
describe('loadCliConfig ideMode', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.UEVO_CLI_IDE_SERVER_PORT;
  });
  ...
  it('should be true when --ide-mode is set and TERM_PROGRAM is vscode', async () => {
    ...
    process.env.UEVO_CLI_IDE_SERVER_PORT = '3000';
    const settings: Settings = {};
    const config = await loadCliConfig(settings, [], 'test-session', argv);
