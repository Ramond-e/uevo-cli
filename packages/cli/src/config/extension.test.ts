...
describe('loadExtensions', () => {
  ...
  beforeEach(() => {
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'uevo-cli-test-workspace-'),
    );
  ...
  it('should load context file path when UEVO.md is present', () => {
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'UEVO.md'),
    ]);
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'UEVO.md'), 'context');
  }
