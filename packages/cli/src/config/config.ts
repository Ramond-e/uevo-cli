...
// Helper function to normalize proxy URL
function normalizeProxyUrl(proxy: string | undefined): string | undefined {
  if (!proxy || proxy.trim() === '') {
    return undefined;
  }
  ...
  // If it doesn't have a protocol, try adding http://
  try {
...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    ...
    .usage(
      '$0 [options]',
      'uEVO CLI - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('model', {
      alias: 'm',
      type: 'string',
      description: `Model`,
      default: process.env.GEMINI_MODEL || DEFAULT_UEVO_MODEL,
    })
    .option('prompt', {
      alias: 'p',
      type: 'string',
    ...
    .option('proxy', {
      type: 'string',
      description:
        'Proxy for uEVO CLIent, like schema://user:password@host:port',
    })
...
export async function loadCliConfig(
  ...
  if (ideMode) {
    ...
    const companionPort = process.env.UEVO_CLI_IDE_SERVER_PORT;
    if (companionPort) {
  ...
  return new Config({
    sessionId,
    embeddingModel: DEFAULT_UEVO_EMBEDDING_MODEL,
    ...
    fileFiltering: {
      ...
      respectuevoignore: settings.fileFiltering?.respectuevoignore,
