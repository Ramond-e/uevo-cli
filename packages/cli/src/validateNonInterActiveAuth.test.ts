...
describe('validateNonInterActiveAuth', () => {
  ...
  beforeEach(() => {
    originalEnvGeminiApiKey = process.env.UEVO_API_KEY;
    originalEnvVertexAi = process.env.GOOGLE_GENAI_USE_VERTEXAI;
    delete process.env.UEVO_API_KEY;
  ...
  afterEach(() => {
    if (originalEnvGeminiApiKey !== undefined) {
      process.env.UEVO_API_KEY = originalEnvGeminiApiKey;
    } else {
      delete process.env.UEVO_API_KEY;
    }
  ...
  it('uses USE_GEMINI if UEVO_API_KEY is set', async () => {
    process.env.UEVO_API_KEY = 'fake-key';
    const nonInteractiveConfig: NonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
    };
    await validateNonInteractiveAuth(undefined, nonInteractiveConfig);
  ...
  it('uses USE_VERTEX_AI if both UEVO_API_KEY and GOOGLE_GENAI_USE_VERTEXAI are set', async () => {
    process.env.UEVO_API_KEY = 'fake-key';
    process.env.GOOGLE_GENAI_USE_VERTEXAI = 'true';
    process.env.GOOGLE_CLOUD_PROJECT = 'test-project';
    process.env.GOOGLE_CLOUD_LOCATION = 'us-central1';
  ...
  it('uses USE_GEMINI if GOOGLE_GENAI_USE_VERTEXAI is false, UEVO_API_KEY is set, and project/location are available', async () => {
