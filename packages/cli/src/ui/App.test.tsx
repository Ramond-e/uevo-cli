...
interface MockServerConfig {
  ...
  getAIClient: Mock<() => AIClient | undefined>;
  getUserTier: Mock<() => Promise<string | undefined>>;
}

// Mock @uevo/uevo-cli-core and its Config class
vi.mock('@uevo/uevo-cli-core', async (importOriginal) => {
  const actualCore =
    await importOriginal<typeof import('@uevo/uevo-cli-core')>();
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        getAllGeminiMdFilenames: vi.fn(() => ['UEVO.md']),
...
describe('App UI', () => {
  ...
  it('should display active file and other context', async () => {
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Open File (my-file.ts) | 1 UEVO.md File');
  });

  it('should display default "UEVO.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(1);
    ...
    expect(lastFrame()).toContain('Using: 1 UEVO.md File');
  });

  it('should display default "UEVO.md" with plural when contextFileName is not set and count is > 1', async () => {
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Using: 2 UEVO.md Files');
  });
  ...
  it('should display UEVO.md and MCP server count when both are present', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(2);
  ...
  it('should display only MCP server count when UEVO.md count is 0', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(0);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
  ...
  describe('with initial prompt from --prompt-interactive', () => {
    it('should submit the initial prompt automatically', async () => {
      ...
      mockConfig.getAIClient.mockReturnValue({
