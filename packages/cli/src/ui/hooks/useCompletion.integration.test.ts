...
import { Config, FileDiscoveryService } from '@uevo/uevo-cli-core';
...
// Mock dependencies
vi.mock('fs/promises');
vi.mock('@uevo/uevo-cli-core', async () => {
  const actual = await vi.importActual('@uevo/uevo-cli-core');
  return {
    ...actual,
...
describe('useCompletion git-aware filtering integration', () => {
  ...
  beforeEach(() => {
    mockFileDiscoveryService = {
      ...
      shoulduevoignoreFile: vi.fn(),
      shouldIgnoreFile: vi.fn(),
      filterFiles: vi.fn(),
      getuevoignorePatterns: vi.fn(),
      projectRoot: '',
      gitIgnoreFilter: null,
      uevoignoreFilter: null,
      isFileIgnored: vi.fn(),
    ...
    mockFileDiscoveryService.shouldIgnoreFile.mockImplementation(
      (path: string, options) => {
        ...
        if (
          ...
          mockFileDiscoveryService.shoulduevoignoreFile
        ) {
          return mockFileDiscoveryService.shoulduevoignoreFile(path);
        }
        return false;
      },
    );
