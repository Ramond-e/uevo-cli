...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const uevoignored =
      respectFileIgnore.respectuevoignore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectuevoignore: true,
      });

    if (gitIgnored || uevoignored) {
      const reason =
        gitIgnored && uevoignored ? 'both' : gitIgnored ? 'git' : 'gemini';
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_gemini_ignore: respectFileIgnore.respectuevoignore,
    },
