...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectuevoignore: () => true,
  ...
  describe('gemini-ignore filtering', () => {
    it('should skip gemini-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.uevoignore'),
        'build/output.js',
      );
      const uevoignoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${uevoignoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${uevoignoredFile} is gemini-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nGemini-ignored: ${uevoignoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .uevoignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.uevoignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed gemini-ignored and valid files', async () => {
    ...
    const query = `@${validFile} @${uevoignoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${validFile} @${uevoignoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${validFile}:\n` },
