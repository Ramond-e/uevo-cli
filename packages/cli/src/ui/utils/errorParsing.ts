...
const getRateLimitErrorMessageGoogleProQuotaFree = (
  currentModel: string = DEFAULT_UEVO_MODEL,
  fallbackModel: string = DEFAULT_UEVO_FLASH_MODEL,
) =>
...
const getRateLimitErrorMessageGooglePaid = (
  ...
  `\nPossible quota limitations in place or slow response times detected. Switching to the ${fallbackModel} model for the rest of this session. We appreciate you for choosing Gemini Code Assist and the uEVO CLI.`;

const getRateLimitErrorMessageGoogleProQuotaPaid = (
  currentModel: string = DEFAULT_UEVO_MODEL,
  fallbackModel: string = DEFAULT_UEVO_FLASH_MODEL,
) =>
  `\nYou have reached your daily ${currentModel} quota limit. You will be switched to the ${fallbackModel} model for the rest of this session. We appreciate you for choosing Gemini Code Assist and the uEVO CLI. To continue accessing the ${currentModel} model today, consider using /auth to switch to using a paid API key from AI Studio at https://aistudio.google.com/apikey`;
      ...
      if (isProQuotaExceededError(error)) {
        return isPaidTier
          ? getRateLimitErrorMessageGoogleProQuotaPaid(
              currentModel || DEFAULT_UEVO_MODEL,
              fallbackModel,
            )
