...
import { SandboxConfig } from '@uevo/uevo-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'uevo-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'uevo-cli-sandbox';
const SANDBOX_PROXY_NAME = 'uevo-cli-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // determine full path for uevo-cli to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under uevo-cli repo
  //
  // note this can only be done with binary linked from uevo-cli repo
  if (process.env.BUILD_SANDBOX) {
    if (!gcPath.includes('uevo-cli/packages/')) {
      console.error(
        'ERROR: cannot build sandbox using installed gemini binary; ' +
          'run `npm link ./packages/cli` under uevo-cli repo to switch to linked binary.',
      );
      process.exit(1);
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the uevo-cli repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify uevo-cli-dev@google.com if the issue persists.';
    console.error(
  ...
  if (process.env.UEVO_API_KEY) {
    args.push('--env', `UEVO_API_KEY=${process.env.UEVO_API_KEY}`);
  }
  ...
  if (process.env.UEVO_CLI_INTEGRATION_TEST === 'true') {
    args.push('--user', 'root');
    userFlag = '--user root';
