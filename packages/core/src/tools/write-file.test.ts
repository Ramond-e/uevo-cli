...
const rootDir = path.resolve(os.tmpdir(), 'uevo-cli-test-root');

// --- MOCKS ---
vi.mock('../core/client.js');
vi.mock('../utils/editCorrector.js');

let mockAIClientInstance: Mocked<GeminiClient>;
...
const mockConfigInternal = {
  ...
  getAIClient: vi.fn(), // Initialize as a plain mock function
  getApiKey: () => 'test-key',
  getModel: () => 'test-model',
  getSandbox: () => false,
  getDebugMode: () => false,
...
describe('WriteFileTool', () => {
  ...
  beforeEach(() => {
    ...
    // Setup AIClient mock
    mockAIClientInstance = new (vi.mocked(GeminiClient))(
      mockConfig,
    ) as Mocked<GeminiClient>;
    vi.mocked(GeminiClient).mockImplementation(() => mockAIClientInstance);

    // Now that mockAIClientInstance is initialized, set the mock implementation for getAIClient
    mockConfigInternal.getAIClient.mockReturnValue(
