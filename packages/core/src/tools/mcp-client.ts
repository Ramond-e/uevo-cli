...
async function handleAutomaticOAuth(
  ...
  try {
    ...
    if (!oauthConfig) {
      console.error(
        `‚ù?Could not configure OAuth for '${mcpServerName}' - please authenticate manually with /mcp auth ${mcpServerName}`,
      );
      return false;
    }
      ...
      if (wwwAuthenticate) {
        ...
        if (mcpServerConfig.url) {
          ...
          try {
            ...
            console.error(
              `‚ù?OAuth discovery failed for '${mcpServerName}' - please authenticate manually with /mcp auth ${mcpServerName}`,
            );
            throw discoveryError;
          }
        } else {
          console.error(
            `‚ù?'${mcpServerName}' requires authentication but no OAuth configuration found`,
          );
          throw new Error(
