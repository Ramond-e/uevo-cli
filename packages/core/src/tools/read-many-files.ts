...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .uevoignore patterns (optional, defaults to true)
   */
  ...
  constructor(private config: Config) {
    const parameterSchema: Schema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .uevoignore',
          type: Type.OBJECT,
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_gemini_ignore: {
              description:
                'Optional: Whether to respect .uevoignore patterns when listing files. Defaults to true.',
              type: Type.BOOLEAN,
            },
          },
        },
...
Use this tool when the user's query implies needing the content of several files simultaneously for context, analysis, or summarization. For text files, it uses default UTF-8 encoding and a '--- {filePath} ---' separator between file contents. Ensure paths are relative to the target directory. Glob patterns like 'src/**/*.js' are supported. Avoid using for single files if a more specific single-file reading tool is available, unless the user specifically requests to process a list containing just one file via this tool. Other binary files (not explicitly requested as image/PDF) are generally skipped. Default excludes apply to common non-text files (except for explicitly requested images/PDFs) and large dependency directories unless 'useDefaultExcludes' is false.`,
  ...
  getDescription(params: ReadManyFilesParams): string {
    ...
    const uevoignorePatterns = this.config
      .getFileService()
      .getuevoignorePatterns();
    const finalExclusionPatternsForDescription: string[] =
      paramUseDefaultExcludes
        ? [...DEFAULT_EXCLUDES, ...paramExcludes, ...uevoignorePatterns]
        : [...paramExcludes, ...uevoignorePatterns];
    ...
    // Add a note if .uevoignore patterns contributed to the final list of exclusions
    if (uevoignorePatterns.length > 0) {
      const geminiPatternsInEffect = uevoignorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (geminiPatternsInEffect > 0) {
        excludeDesc += ` (includes ${geminiPatternsInEffect} from .uevoignore)`;
      }
    }
  ...
  async execute(
    ...
    const fileFilteringOptions = {
      ...
      respectuevoignore:
        ...
        defaultFileIgnores.respectuevoignore, // Use the property from the returned object
    };
    ...
    try {
      ...
      const finalFilteredEntries = fileFilteringOptions.respectuevoignore
        ? fileDiscovery
            .filterFiles(
      ...
      for (const absoluteFilePath of entries) {
        ...
        if (
          fileFilteringOptions.respectuevoignore &&
      ...
      if (uevoignoredCount > 0) {
        skippedFiles.push({
          path: `${uevoignoredCount} file(s)`,
