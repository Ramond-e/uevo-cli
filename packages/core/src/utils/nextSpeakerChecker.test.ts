...
import { GeminiChat } from '../core/uevoChat.js';

// Mock AIClient and Config constructor
...
describe('checkNextSpeaker', () => {
  ...
  beforeEach(() => {
    ...
    mockAIClient = new GeminiClient(mockConfigInstance);
  ...
  it('should return null if history is empty', async () => {
    ...
    expect(mockAIClient.generateJson).not.toHaveBeenCalled();
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockAIClient.generateJson as Mock).mockResolvedValue(mockApiResponse);
    ...
    expect(mockAIClient.generateJson).toHaveBeenCalledTimes(1);
  });
  ...
  it('should return null if geminiClient.generateJson throws an error', async () => {
    ...
    (mockAIClient.generateJson as Mock).mockRejectedValue(
      new Error('API Error'),
    );
  ...
  it('should return null if geminiClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    ...
    (mockAIClient.generateJson as Mock).mockResolvedValue({
  ...
  it('should call generateJson with DEFAULT_UEVO_FLASH_MODEL', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    const mockApiResponse: NextSpeakerResponse = {
    ...
    await checkNextSpeaker(chatInstance, mockAIClient, abortSignal);

    expect(mockAIClient.generateJson).toHaveBeenCalled();
    const generateJsonCall = (mockAIClient.generateJson as Mock).mock
      .calls[0];
    expect(generateJsonCall[3]).toBe(DEFAULT_UEVO_FLASH_MODEL);
  });
});
