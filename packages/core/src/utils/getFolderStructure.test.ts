...
`.trim();
  ...
  describe('with uevoignore', () => {
    it('should ignore uevoignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.uevoignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectuevoignore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.uevoignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
