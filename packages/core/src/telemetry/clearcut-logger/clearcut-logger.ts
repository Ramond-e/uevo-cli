...
export class ClearcutLogger {
  ...
  createLogEvent(name: string, data: object[]): object {
    ...
    data.push({
      UEVO_CLI_key: EventMetadataKey.UEVO_CLI_GOOGLE_ACCOUNTS_COUNT,
      value: totalAccounts.toString(),
    });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const logEvent: any = {
      console_type: 'uevo_cli',
      application: 102,
      event_name: name,
  ...
  logStartSessionEvent(event: StartSessionEvent): void {
    ...
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_START_SESSION_MODEL,
        value: event.model,
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_SESSION_ID,
        value: this.config?.getSessionId() ?? '',
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_START_SESSION_EMBEDDING_MODEL,
        value: event.embedding_model,
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_START_SESSION_SANDBOX,
        value: event.sandbox_enabled.toString(),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_START_SESSION_CORE_TOOLS,
        value: event.core_tools_enabled,
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_START_SESSION_APPROVAL_MODE,
        value: event.approval_mode,
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_START_SESSION_API_KEY_ENABLED,
      ...
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_START_SESSION_TELEMETRY_ENABLED,
        value: event.telemetry_enabled.toString(),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_START_SESSION_TELEMETRY_LOG_USER_PROMPTS_ENABLED,
        value: event.telemetry_log_user_prompts_enabled.toString(),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_SURFACE,
        value: surface,
      },
    ];
  ...
  logNewPromptEvent(event: UserPromptEvent): void {
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_USER_PROMPT_LENGTH,
      ...
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_PROMPT_ID,
        value: JSON.stringify(event.prompt_id),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_AUTH_TYPE,
  ...
  logToolCallEvent(event: ToolCallEvent): void {
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_CALL_NAME,
      ...
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_CALL_DECISION,
        value: JSON.stringify(event.decision),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_CALL_SUCCESS,
        value: JSON.stringify(event.success),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_CALL_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_ERROR_MESSAGE,
        value: JSON.stringify(event.error),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_TOOL_CALL_ERROR_TYPE,
  ...
  logApiRequestEvent(event: ApiRequestEvent): void {
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_REQUEST_MODEL,
  ...
  logApiResponseEvent(event: ApiResponseEvent): void {
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_RESPONSE_MODEL,
        value: JSON.stringify(event.model),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_PROMPT_ID,
      ...
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_RESPONSE_STATUS_CODE,
        value: JSON.stringify(event.status_code),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_RESPONSE_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_ERROR_MESSAGE,
        value: JSON.stringify(event.error),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_API_RESPONSE_INPUT_TOKEN_COUNT,
        value: JSON.stringify(event.input_token_count),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_API_RESPONSE_OUTPUT_TOKEN_COUNT,
        value: JSON.stringify(event.output_token_count),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_API_RESPONSE_CACHED_TOKEN_COUNT,
        value: JSON.stringify(event.cached_content_token_count),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_API_RESPONSE_THINKING_TOKEN_COUNT,
        value: JSON.stringify(event.thoughts_token_count),
      },
      {
        UEVO_CLI_key:
          EventMetadataKey.UEVO_CLI_API_RESPONSE_TOOL_TOKEN_COUNT,
  ...
  logApiErrorEvent(event: ApiErrorEvent): void {
    const data = [
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_ERROR_MODEL,
        value: JSON.stringify(event.model),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_PROMPT_ID,
      ...
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_ERROR_TYPE,
        value: JSON.stringify(event.error_type),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_ERROR_STATUS_CODE,
        value: JSON.stringify(event.status_code),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_API_ERROR_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_AUTH_TYPE,
  ...
  logLoopDetectedEvent(event: LoopDetectedEvent): void {
    const data = [
      ...
      {
        UEVO_CLI_key: EventMetadataKey.UEVO_CLI_LOOP_DETECTED_TYPE,
