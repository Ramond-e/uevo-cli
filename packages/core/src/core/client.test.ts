...
describe('uEVO CLIent (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the AIClient constructor kicks off an async process (startChat)
    ...
    mockConfigObject.getAIClient.mockReturnValue(client);
  ...
  // was not being detected as called by the AIClient instance.
  // This likely points to a subtle issue in how the GoogleGenerativeAI class constructor
  // and its instance methods are mocked and then used by the class under test.
  // For future debugging, ensure that the `this.client` in `AIClient` (which is an
  ...
  // For future debugging, ensure `this.client.models.generateContent` in `AIClient` correctly
